...
// 조건문의 종류 : if, else if, if-else, switch
...
// else문, else-if문이 복잡하게 겹쳐질 때 사용하ㅕㄴ 코드를 간결하고 이해하기 쉽게 만든다.
...
// switch문은 조어진 식의 결과값과 case에 명시된 값들을 순차적으로 비교
...
조건 ? 표현식1(조건이 참일 경우) : 표현식2(조건이 거짓일 경우)

// 조건 연산자 : 주어진 조건을 평가한 후,
// 그 결과 true이면 표현식1을, false이면 표현식2를 실행 : if else 문의 로직과 동일
...
// 아래 4가지 식 중 다른값이 출력되는 식은?

true ? alert('출력A') : alert('출력B');// 출력A
false ? alert('출력B') : alert('출력A'); // 출력A
true || alert('출력A'); // 출력안됨 : true값이 앞에 나오면 두번째 피연산자는 무시함
alert('출력A') || true; // 출력A
true && alert('출력A'); // 출력A
...
if (numberOne < 10 || numberTwo < 10) alert('숫자 두 개 중에서 최소한 하나는 10미만입니다.')
