...
console.log(Math.random()); // 0과 1사이의 숫자를 랜덤형식(무작위)으로 내뱉어줌
console.log(Math.floor(Math.random() * 11)); // 0과 10 사이의 숫자를 정수로 내뱉어줌
...
let obj = { name: 'Jeong Bin', age: 28};
console.log(obj)
let json = JSON.stringify(obj); // 객체를 JSON 문자열로 변환
console.log(json);

let parsed = JSON.parse(json);
console.log(parsed); // JSON 형식이 객체 형식으로 반환

//! 자바스크립트 객체 프로토타입

//? 1. 프로토타입
// 그 객체를 기반으로 생성된 다른 객체들이 공통적으로 사용할 수 있는 속성과 메소드를 제공합니다.
...
console.log(academy.constructor === Person); // true
console.log(academy.constructor.prototype === Person.prototype); // true
...
// 객체: 키와 값으로 이루어진 구조
// 키는 일반적으로 문자열 사용, 값은 어떤 데이터 타입이든 가능
